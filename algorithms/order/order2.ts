...
const sketch = (p: p5) => {
	...
	const tremblingBox = (w: number, h: number, d: number, tremble = 0.05) => {
		...
		for (let i = 0; i < 3; i++) {
			...
			p.rotateY(p.random(-tremble, tremble))
			p.rotateZ(p.random(-tremble, tremble))
			p.box(w, h, d)
			p.pop()
		}
	}
			...
			if (rnd === 0) {
				...
				for (let j = 0; j < sub; j++) {
					...
					if (hor) {
						...
						p.stroke(col)
						p.noFill()
						// Use trembling box instead of regular box
						tremblingBox(dd, r.z, hh)
						p.pop()
					} else {
						const hhh = hh * p.random(1)
						p.push()
